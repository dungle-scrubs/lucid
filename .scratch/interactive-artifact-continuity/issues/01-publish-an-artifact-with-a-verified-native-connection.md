...
- [ ] Verified native registration binds only the exact native identity, working folder, interface and corroborated owner; stale, ambiguous, subagent, or managed-child claims cannot bind.
- [ ] Binding is durable and race-safe against lifecycle changes. An open but non-listening native owner has the agreed message; unavailable setup keeps the artifact readable.
