...
## Native startup decision

The isolated Codex 0.154.0 + local Qwen probe shows that a trusted, enabled SessionStart/resume hook runs only after the resumed terminal receives its first user turn. Opening the terminal alone stayed idle and emitted no hook event. A later harmless user turn produced the exact-session hook event. This does not establish model adherence to the hook's context; the model returned the seed marker. Evidence, limits and owned cleanup are in ignored reconnect-startup-observation.md.

RFC 26's strict HCN interactive operation currently forbids a prompt. Before automatic hook selection can complete unattended reconnect, decide whether to permit one fixed Lucid startup instruction through native resume, or require a person's first turn. Recommended amendment: a narrowly specified startup instruction identifies the reserved conversation and asks the same session to resume listening; it carries no saved feedback or receipt claim. Keep session identity, native settings, reservation and verified-child admission unchanged. Native support and acceptance of that amended path must be verified before activation. No bootstrap behavior has been implemented.

The user approved the recommended startup instruction and autonomous continuation. RFC 26 v5 specifies the bounded HCN startup prompt, fixed Lucid command, no feedback payload or receipt inference, and conservative refusal when selection races provenance or lease release. Implementation seams remain HCN CLI/native process normalization, HarnessRunner/owned reconnect launch, and the existing verified in-session selection/listener. Native acceptance must establish the resulting listener, not merely an opened terminal.

## Fixed startup checkpoint

RFC 26 v6 resolves the Muse v5 review and permits the approved startup instruction. HCN now accepts one bounded optional startup prompt, distinguishes option positions from prompt values, rejects duplicate/invalid text before spawn, and renders the native prompt through its interface descriptor. Lucid always supplies its fixed exact-record resume-listen command through the existing consumed invocation. Shared POSIX quoting preserves executable and root paths; no queued input, offer or receipt is sent in the startup turn. Invalid startup and unavailable support have specific held explanations.

The isolated Codex 0.154.0 / local Qwen native lane resumed the same session twice sequentially, with verified owned cleanup between launches. Actual SessionStart registration, one resume-listen tool command, Stop listening, and durable reservation fulfillment were observed. The second native turn retained the preceding interactive turn's model, effort, folder, approval policy and filesystem/network policy. Each startup command required a native on-request approval, granted once for this disposable test; unattended startup is not claimed when native policy asks. The initial binding was a controlled fixture with real native process provenance, not a new proof of artifact publication. No feedback input or offer was fabricated. Evidence and limits are under ignored startup-v5-native-* artifacts.

Four Muse code-review axes completed; the remaining duplicate quoting call was consolidated. Public reconnect reservation/wait CLI, browser controls, dependency activation and other interface acceptance remain pending. The bound runtime remains fenced.

## Public terminal command seam

Machine-made continuation under RFC 26: `lucid reconnect CONVERSATION` is a terminal interaction, with inherited native I/O and concise progress on stderr. Machine-readable detection remains `lucid connection status CONVERSATION --json`; reconnect does not mix a JSON result stream with native terminal bytes. It accepts no harness, model, session, folder or prompt override. Non-terminal callers receive an instruction before any reservation. The command uses exact record addressing, the host's durable request and cancellation operations, the shared record follower while admission is blocked, and runNativeReconnect for its single launch. Duplicate requests report the existing request. Ctrl+C withdraws only the command's pre-launch request; after launch, normal owned cleanup applies. A prior requested wait may be reconciled only through the host's corroborated requester-exit operation. No intended or uncertain attempt is reissued.

Tests remain at the accepted public command/host/HarnessRunner seams: (1) exact target and one invocation with a durable request before startup, (2) actual executor lock/current response blocks launch until released, (3) cancellation preserves feedback/current work with zero native invocation, (4) duplicate and departed requester handling, (5) missing terminal, alive/unknown owner and uncertain launch retain the record without fallback. The shared follower is an injected filesystem/timer boundary for deterministic trigger ordering; actual process/lock probes supplement it.

## Public command checkpoint

The terminal command now resolves the exact record, requires a terminal before reservation, preserves existing live waiters, and reconciles a departed pre-launch requester only through the guarded host operation. It follows the record while execution/admission is blocked, owns no executor lease during that wait, and makes one guarded native invocation. Cancellation withdraws only its pre-launch request. Closed, refused and uncertain terminal results stay distinct. Alive authors get the exact existing-session listening instruction; unknown owners and forbidden overrides make no reservation or launch.

Public tests cover real kernel-lock waiting, cancellation with saved feedback, duplicate suppression, live versus genuinely exited foreign requesters, absent terminal, unknown ownership, pre-aborted commands, forbidden overrides, and uncertain native results. The actual compiled CLI resumed the same disposable Codex/local-Qwen session, ran its fixed command after one native approval, reached listening and durable fulfillment, and closed with verified exit 0/cleanup. Model, effort, original folder and approval/sandbox policies matched the prior interactive turn. This does not claim unattended approval or full browser/headless handoff acceptance. All task panes are closed. Four Muse axes and root dispositions are retained under ignored reconnect-command-* evidence. Browser controls, bound-runtime activation and other native lanes remain pending.
