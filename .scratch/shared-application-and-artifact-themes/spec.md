...
The user settled the remaining Settings decision. The RFC remains Draft;
these local tickets do not change its acceptance status or authorize
implementation. There is no parent tracker issue to modify.
...
All four issues are local drafts. Nothing has been published to a shared tracker.
