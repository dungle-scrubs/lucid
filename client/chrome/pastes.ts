 ...
 *
 * Tab-local by design. The staged map lives in this module, not the log - a
 * reload before sending forgets the content along with the draft that
 * referenced it, and a placeholder edited into something else simply sends
 * as the text it became, exactly as typed.
 */
...
export const stagePaste = (text: string): string => {
  counter += 1;
...
export const expandPastes = (text: string): string => {
  let out = text;
  for (const [placeholder, content] of staged) {
    // A function replacement: a string one would reinterpret `$&` and friends
    // inside the pasted content as substitution patterns.
...
export const consumePastes = (text: string): void => {
...
/**
 * The composer half: fold a large text paste into a placeholder at the caret.
 * Shared by every textarea that takes feedback text. Small pastes fall
 * through untouched, as do file pastes (their text/plain slot is empty).
 */
export const collapseTextPaste = (e: ClipboardEvent<HTMLTextAreaElement>): void => {
  ...
  // the caret, fires the input event a React-controlled textarea listens for,
  // and keeps the undo stack. Engines that refuse it get the manual splice.
