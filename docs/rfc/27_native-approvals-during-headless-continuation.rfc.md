...
The implementation also bounds lifetime request identities and recorded decisions to 4096 each within one process. Reused native RPC IDs fail as unsupported even after a clear, because a delayed clear carries no generation with which to distinguish a later request. These are machine-made transport bounds under the accepted autonomous implementation scope; they add no cross-process state or approval authority.
