...
const apply=Reflect.apply,descriptor=Object.getOwnPropertyDescriptor,parentPrototype=Object.getPrototypeOf;
...
const elementTag=getter(Element.prototype,"tagName"),elementShadowRoot=getter(Element.prototype,"shadowRoot"),elementClientWidth=getter(Element.prototype,"clientWidth"),elementClientHeight=getter(Element.prototype,"clientHeight"),elementScrollWidth=getter(Element.prototype,"scrollWidth"),elementScrollHeight=getter(Element.prototype,"scrollHeight"),elementRect=Element.prototype.getBoundingClientRect,elementAnimate=Element.prototype.animate,elementAttribute=Element.prototype.getAttribute,elementSetAttribute=Element.prototype.setAttribute,elementRemoveAttribute=Element.prototype.removeAttribute,elementQuery=Element.prototype.querySelectorAll,elementRemove=Element.prototype.remove;
const documentQuery=Document.prototype.querySelectorAll,documentWalker=Document.prototype.createTreeWalker,documentCreate=Document.prototype.createElement,walkerNext=TreeWalker.prototype.nextNode,showElement=NodeFilter.SHOW_ELEMENT,showText=NodeFilter.SHOW_TEXT,documentRoot=getter(Document.prototype,"documentElement"),documentBody=getter(Document.prototype,"body"),documentReady=getter(Document.prototype,"readyState"),documentHead=getter(Document.prototype,"head"),documentImportNode=Document.prototype.importNode;
...
const eventAdd=EventTarget.prototype.addEventListener,eventRemove=EventTarget.prototype.removeEventListener,eventTarget=getter(Event.prototype,"target"),messageData=getter(MessageEvent.prototype,"data"),messagePortClose=MessagePort.prototype.close,messagePortPost=MessagePort.prototype.postMessage,messagePortStart=MessagePort.prototype.start,defineProperty=Object.defineProperty,freeze=Object.freeze,ownNames=Object.getOwnPropertyNames,mapClear=Map.prototype.clear,mapGet=Map.prototype.get,mapSet=Map.prototype.set,setAdd=Set.prototype.add,setDelete=Set.prototype.delete,setForEach=Set.prototype.forEach,setSize=getter(Set.prototype,"size"),weakGet=WeakMap.prototype.get,weakSet=WeakMap.prototype.set,weakDelete=WeakMap.prototype.delete,weakSetAdd=WeakSet.prototype.add,weakSetDelete=WeakSet.prototype.delete,weakSetHas=WeakSet.prototype.has;
const arrayConstructor=Array,mapConstructor=Map,setConstructor=Set,weakMapConstructor=WeakMap,numberConstructor=Number,stringConstructor=String,regexpConstructor=RegExp,mathObject=Math,arrayPrototype=Array.prototype,mapPrototype=Map.prototype,setPrototype=Set.prototype,weakMapPrototype=WeakMap.prototype,stringPrototype=String.prototype,regexpPrototype=RegExp.prototype;
const intrinsicEntry=(owner,key)=>({descriptor:descriptor(owner,key),key,owner});
const arrayIteratorPrototype=parentPrototype(apply(arrayPrototype[Symbol.iterator],[],[])),stringIteratorPrototype=parentPrototype(apply(stringPrototype[Symbol.iterator],"",[]));
const proofIntrinsicEntries=[intrinsicEntry(arrayConstructor,"from"),intrinsicEntry(arrayConstructor,"isArray"),intrinsicEntry(arrayPrototype,Symbol.iterator),intrinsicEntry(arrayPrototype,"filter"),intrinsicEntry(arrayPrototype,"flatMap"),intrinsicEntry(arrayPrototype,"map"),intrinsicEntry(arrayPrototype,"push"),intrinsicEntry(arrayPrototype,"shift"),intrinsicEntry(arrayPrototype,"slice"),intrinsicEntry(arrayPrototype,"some"),intrinsicEntry(arrayPrototype,"splice"),intrinsicEntry(arrayIteratorPrototype,"next"),intrinsicEntry(mapPrototype,"clear"),intrinsicEntry(mapPrototype,"get"),intrinsicEntry(mapPrototype,"set"),intrinsicEntry(setPrototype,"add"),intrinsicEntry(setPrototype,"has"),intrinsicEntry(weakMapPrototype,"get"),intrinsicEntry(weakMapPrototype,"set"),intrinsicEntry(numberConstructor,"isFinite"),intrinsicEntry(numberConstructor,"isSafeInteger"),intrinsicEntry(mathObject,"ceil"),intrinsicEntry(mathObject,"max"),intrinsicEntry(mathObject,"min"),intrinsicEntry(stringPrototype,Symbol.iterator),intrinsicEntry(stringPrototype,"codePointAt"),intrinsicEntry(stringPrototype,"replace"),intrinsicEntry(stringPrototype,"replaceAll"),intrinsicEntry(stringPrototype,"trim"),intrinsicEntry(stringIteratorPrototype,"next"),intrinsicEntry(regexpPrototype,"exec"),intrinsicEntry(regexpPrototype,Symbol.replace)];
const descriptorField=(record,key)=>record===undefined?undefined:descriptor(record,key)?.value;
const sameDescriptor=(left,right)=>left!==undefined&&right!==undefined&&descriptorField(left,"configurable")===descriptorField(right,"configurable")&&descriptorField(left,"enumerable")===descriptorField(right,"enumerable")&&descriptorField(left,"get")===descriptorField(right,"get")&&descriptorField(left,"set")===descriptorField(right,"set")&&descriptorField(left,"value")===descriptorField(right,"value")&&descriptorField(left,"writable")===descriptorField(right,"writable");
const proofRealmTrusted=()=>{if(descriptor(window,"Array")?.value!==arrayConstructor||descriptor(window,"Map")?.value!==mapConstructor||descriptor(window,"Set")?.value!==setConstructor||descriptor(window,"WeakMap")?.value!==weakMapConstructor||descriptor(window,"Number")?.value!==numberConstructor||descriptor(window,"String")?.value!==stringConstructor||descriptor(window,"RegExp")?.value!==regexpConstructor||descriptor(window,"Math")?.value!==mathObject||descriptor(arrayConstructor,"prototype")?.value!==arrayPrototype||descriptor(mapConstructor,"prototype")?.value!==mapPrototype||descriptor(setConstructor,"prototype")?.value!==setPrototype||descriptor(weakMapConstructor,"prototype")?.value!==weakMapPrototype||descriptor(stringConstructor,"prototype")?.value!==stringPrototype||descriptor(regexpConstructor,"prototype")?.value!==regexpPrototype)return false;for(let index=0;index<proofIntrinsicEntries.length;index+=1){const entry=proofIntrinsicEntries[index];if(!sameDescriptor(descriptor(entry.owner,entry.key),entry.descriptor))return false}return true};
const createTrustedMap=()=>{const map=new mapConstructor();return{clear:()=>apply(mapClear,map,[]),get:(key)=>apply(mapGet,map,[key]),set:(key,value)=>{apply(mapSet,map,[key,value])}}};
const createTrustedWeakMap=()=>{const map=new weakMapConstructor();return{get:(key)=>apply(weakGet,map,[key]),set:(key,value)=>{apply(weakSet,map,[key,value])}}};
...
const boundedText=(element,limit,maxNodes,deadline)=>{const walker=apply(documentWalker,document,[element,showText]);let result="",visited=0,previousParent=null,current=apply(walkerNext,walker,[]);while(current&&result.length<limit&&visited<maxNodes&&clock()<=deadline){const currentValue=apply(nodeValue,current,[])??"";const currentParent=apply(nodeParent,current,[]);/* A heading's number usually lives in its own element ("<span>1.</span>Context"), where the gap a reader sees is CSS, not text. Concatenating raw nodes fused them into "1.Context" in the outline. Join across an element boundary with one space unless either side already brings whitespace. */if(result!==""&&currentParent!==previousParent&&!/s$/.test(result)&&!/^s/.test(currentValue)&&result.length<limit)result+=" ";result+=currentValue.slice(0,limit-result.length);previousParent=currentParent;visited+=1;current=apply(walkerNext,walker,[])}return{complete:current===null||result.length>=limit,examinedNodes:visited,value:result}};
const styleCallbacks=new Set();let styleNotificationPending=false;const notifyStyle=()=>{if(styleNotificationPending||apply(setSize,styleCallbacks,[])===0)return;styleNotificationPending=true;enqueueMicrotask(()=>{styleNotificationPending=false;apply(setForEach,styleCallbacks,[(callback)=>callback()])})};
...
createMap:createTrustedMap,createWeakMap:createTrustedWeakMap,
proofRealmTrusted,
...
isSettled:()=>{if(apply(documentReady,document,[])!=="complete"||fontStatus&&apply(fontStatus,fonts,[])!=="loaded")return false;const images=apply(documentQuery,document,["img"]),length=apply(nodeListLength,images,[]);for(let index=0;index<length;index+=1){const image=apply(nodeListItem,images,[index]);if(image&&!apply(imageComplete,image,[]))return false}return true},
now:clock,
...
importNode:(node,deep)=>apply(documentImportNode,document,[node,deep]),
removeArtifactStyles:()=>{const head=apply(documentHead,document,[]);if(!head)return;const sheets=apply(elementQuery,head,["[data-lucid-artifact-style]"]);const length=apply(nodeListLength,sheets,[]);for(let index=0;index<length;index+=1){const sheet=apply(nodeListItem,sheets,[index]);if(sheet)apply(elementRemove,sheet,[])}},
appendArtifactStyle:(source,onLinkedSheetLoad)=>{const head=apply(documentHead,document,[]);if(!head)return;const clone=apply(documentImportNode,document,[source,true]);apply(elementSetAttribute,clone,["data-lucid-artifact-style","true"]);if(onLinkedSheetLoad)apply(eventAdd,clone,["load",onLinkedSheetLoad,{once:true}]);apply(nodeAppend,head,[clone])}
};
