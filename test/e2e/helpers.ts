 ...
 * `fixtures.ts`, `routes.ts`, `windows.ts`, plus `kill-server.ts` and
 * `cli-result.ts`. Suites import from here; the modules are where the code
 * lives and where a change belongs.
 *
...
export { BINARY, MAIN, makeCli, waitTimeoutSeconds, type Cli, type CliOptions } from "./cli.ts";
export * from "./hub.ts";
